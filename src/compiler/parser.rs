...
impl Parser {
    pub fn parse(&mut self) -> Result<StatementsNode, String>{
    ...
    fn statements(
        ...
        loop {
            let statement_node = self.statement(return_error_if_not_matched)?;
            if statement_node.0{
                break;
            }
    ...
    pub fn define_if_statement(&mut self) -> Result<(bool, DefineIfStatementNode), String>{
